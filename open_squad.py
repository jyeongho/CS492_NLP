...
# Modified version. To apply retrospective paper, add 'is_impossible', 'pq_end_pos' to SquadFeature.
...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    for span in spans:
        ...
        question_end_index = span["truncated_query_with_special_tokens_length"] - 1
...
def squad_convert_examples_to_features(
    ...
    if threads == 1:
        ...
        features = []
        for eg in tqdm(examples, total=len(examples), position=0, leave=True, desc="convert squad examples to features"):
        ...
        with Pool(threads, initializer=squad_convert_example_to_features_init, initargs=(tokenizer,)) as p:
            ...
            features = list(
                tqdm(
                    ...
                    total=len(examples), position=0, leave=True,
    ...
    example_index = 0
    for example_features in tqdm(features, total=len(features), position=0, leave=True, desc="add example index and unique id"):
    ...
    if return_dataset == "pt":
        ...
        # Add is_impossible, pq_end_pos features into Dataset for retrospective paper.
        all_is_impossible = torch.tensor([int(f.is_impossible) for f in features], dtype=torch.long)
...
class SquadProcessor(DataProcessor):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        for entry in tqdm(input_data[:]):
            ...
            ##################Sort paragraph with word-based similarity#####################

            q_word = []
            para_word_cnt = []
            for i in question_text.split(' '): ## Tokenize question
                if len(i) > 2:
                    word1 = i[:-1]
                    word2 = i
                    word3 = i[1:]
                    q_word.append((word1,word2,word3))
                elif len(i) == 2:
                    q_word.append((i, i, i))

            for p in entry["paragraphs"]: ## Get similarity of each paragraph
                para_cnt = 0
                for sent in str(p["contents"]).split('.'):
                    word_cnt = 0
                    for word in q_word:
                        if (str(word[0]) in str(sent)) or (str(word[1]) in str(sent)) or (str(word[2]) in str(sent)):
                            word_cnt += 1
                    if word_cnt > para_cnt:
                        para_cnt = word_cnt
                if str(p["source"]) == 'kdc':
                    para_cnt += 1
                para_word_cnt.append(para_cnt)

            sorted_paragraph = [x for y, x in sorted(zip(para_word_cnt, entry["paragraphs"]),key = lambda k: k[0], reverse=True)] ## Return sorted list of paragraph
            
            ####################################################################################


            for pi, paragraph in enumerate(sorted_paragraph):
...
class SquadResult_with_score(object):
    """
    Constructs a SquadResult which can be used to evaluate a model's output on the SQuAD dataset.
    Add ext score argument for retrospective-reader.
    Args:
        unique_id: The unique identifier corresponding to that example.
        start_logits: The logits corresponding to the start of the answer
        end_logits: The logits corresponding to the end of the answer
        score_ext: The ext score
        label: is_impossible value. It is used for debugging and calculating threshold
    """

    def __init__(self, unique_id, start_logits, end_logits, score_ext, label):
        self.start_logits = start_logits
        self.end_logits = end_logits
        self.unique_id = unique_id
